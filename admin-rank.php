...
<div class="progress">
<?php if ($progress < 100) {?>
    <div class="progress-bar progress-bar-striped progress-bar-animated bg-success" style="width:<?php echo $progress; ?>%"><?php echo (int)$progress; ?>%</div>
<?php } else { ?>
    <div class="progress-bar bg-success" style="width: 100%">Hết thời gian làm bài</div>;
<?php
}
?>
</div>
...
<br>
<br>
<table  class="table table-bordered table-hover" > <thead class="thead-light"><tr><th width="1"><b><center>Rank</center></b></th><th><b>Name</b></th><th width="1"><b>SLV.</b></th><th width="1"><b>TIME</b></th>
<?php
// so luong bai tap
$slbt        = 0;
$directorydb = 'debai/';
...
while ($it1->valid()) {
    if (!$it1->isDot()) {
        ...
        if ($dd[$Tenbai] != "true") {
            ...
            $nameb[$slbt] = $Tenbai;
            $dd[$Tenbai]  = "true";
            echo "<th class='text-center' width='80'><a target='_blank' href='" . "http://" . $_SERVER['HTTP_HOST'] . "/themis/" . $directorydb . $it1->getSubPathName() . "' data-title='".$Tenchitiet."'>" . $Tenbai . "</a></th>";
        }
    }
    $it1->next();
}
